...
$stmt = $conn->prepare("SELECT * FROM congnoncc WHERE macongnoNCC = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $conn->prepare("UPDATE congnoncc SET maNCC = ?, maNV = ?, tongno = ?, tongtiendatra = ?, ghichu = ? WHERE macongnoNCC = ?");
            $stmt->bind_param('ssddss', $maNCC, $maNV, $tongno, $tongtiendatra, $ghichu, $id);
            $stmt->execute();
            $stmt->close();
            
            flash_set('success', "Cap nhat cong no nha cung cap thanh cong.");
            header('Location: index.php');
            exit;
        } catch (Throwable $e) {
            error_log('[CongNoNCC-Edit] ' . $e->getMessage());
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="maNCC" class="form-select" required>
                    ...
                    $selNCC = $_POST['maNCC'] ?? $congno['maNCC'];
