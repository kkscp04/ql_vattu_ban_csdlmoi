...
if (($header['trangthai'] ?? '') === KK_STATUS_HOAN_THANH) {
    flash_set('warning', "Phieu kiem ke '$id' da hoan thanh, khong duoc sua.");
    header('Location: index.php');
    exit;
}

$oldRows = kk_fetch_detail_rows($conn, $id);
$lotOptions = kk_load_available_lots($conn, array_column($oldRows, 'maLo'));
$rows = $oldRows;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $maNV = trim($_POST['maNV_Lap'] ?? '');
    $status = trim($_POST['trangthai'] ?? ($header['trangthai'] ?? KK_STATUS_DANG_KIEM_KE));
    $ghichu = trim($_POST['ghichu'] ?? '');
    $snapshotMap = [];
    foreach ($oldRows as $oldRow) {
        $snapshotMap[$oldRow['maLo']] = $oldRow['soLuongHeThong'];
    }
    $rows = kk_collect_post_rows($conn, $snapshotMap);
    $errors = kk_validate($conn, $id, $maNV, $status, $rows, false, array_column($oldRows, 'maLo'));

    if (empty($errors)) {
        try {
            kk_save($conn, $id, $maNV, $status, $ghichu, $rows, $header, $oldRows);
            flash_set('success', "Da cap nhat phieu kiem ke '$id'.");
            header('Location: index.php');
            exit;
        } catch (Throwable $e) {
            error_log('[KiemKe-Edit] ' . $e->getMessage());
            $errors[] = 'Loi khi cap nhat phieu kiem ke. Vui long thu lai.';
        }
    }
...
<div class="card shadow p-4">
    <h4 class="fw-bold mb-3">Sua phieu kiem ke #<?= htmlspecialchars($id) ?></h4>
    ...
    <form method="POST">
        <div class="row mb-3">
            ...
            <div class="col-md-3">
                ...
                <select name="maNV_Lap" class="form-select" required>
                    ...
                    $selNV = $_POST['maNV_Lap'] ?? ($header['maNV_Lap'] ?? '');
            ...
            <div class="col-md-3">
                ...
                <select name="trangthai" class="form-select">
                    <?php $currentStatus = $_POST['trangthai'] ?? ($header['trangthai'] ?? KK_STATUS_DANG_KIEM_KE); ?>
            ...
            <div class="col-md-3">
                <label>Thoi gian kiem ke</label>
                <input type="text" class="form-control" value="<?= !empty($header['thoigiankiemke']) ? date('d/m/Y H:i', strtotime($header['thoigiankiemke'])) : '' ?>" readonly>
            </div>
            <div class="col-12 mt-3">
                <label>Ghi chu</label>
                <input type="text" name="ghichu" class="form-control" value="<?= htmlspecialchars($_POST['ghichu'] ?? ($header['ghichu'] ?? '')) ?>">
            </div>
        </div>
            ...
            <tbody>
                <?php foreach ($rows as $row): ?>
                    <tr>
                        ...
                        <td><input type="number" name="soLuongHeThong[]" class="form-control so-he-thong" step="0.01" value="<?= htmlspecialchars((string) $row['soLuongHeThong']) ?>" readonly></td>
        ...
        <div>
            <button class="btn btn-warning"><i class="fas fa-save"></i> Cap nhat</button>
...
const initialRows = <?= json_encode(array_values($rows), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
...
function fillRow(row) {
    ...
    if (lot && row.querySelector('.so-he-thong').value === '') {
        row.querySelector('.so-he-thong').value = Number(lot.soLuongHeThong).toFixed(2);
    }
    calcDiff(row);
}
...
function rowHasDuplicateLot(currentRow) {
    ...
    tableBody.querySelectorAll('tr').forEach((row) => {
        ...
        if (row.querySelector('.lot-select').value === selectedLot) found = true;
    });
    return found;
}
...
function addRow() {
    ...
    row.querySelectorAll('input').forEach((input) => {
        if (input.classList.contains('so-thuc-te')) input.value = '0';
        else input.value = '';
    });
...
tableBody.querySelectorAll('tr').forEach((row, index) => {
    ...
    if (selectedLot) select.value = selectedLot;
    fillRow(row);
});
</script>
